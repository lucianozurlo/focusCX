...
class fcx_Before_After {
   constructor($obj = null) {
      ...
      this.$el = {
         $wrap: $obj,
         $before: jQuery('<div class="fcx-before-after-img fcx-before-img"/>').appendTo($obj),
         $after: jQuery('<div class="fcx-before-after-img fcx-after-img-wrap"/>').appendTo($obj),
         $divider: jQuery(
            '<div class="fcx-before-after-divider">\
      ...
      this.$el.$after.children('img').wrap('<div class="fcx-after-img"/>');
      this.$el.$after
         .children('.fcx-after-img')
         .css('background-image', 'url(' + this.$el.$wrap.attr('data-img-after') + ')');
...
const baw_axis = new fcx_PanAxis(10);
...
class fcx_Menu {
   constructor(_core = null) {
      ...
      this.core.$dom.header.append('<span class="fcx-active-menu-ind"></span>');
      ...
      // Create Mobile Menu
      this.mobile_menu = new fcx_Mobile_Menu(this.$nav);
   }
   layout() {
      const _self = this;
      let active_pos = '-100%';
...
class fcx_Mobile_Menu {
   constructor($obj) {
      ...
      this.$nav = jQuery('<nav class="fcx-mobile-nav"/>').appendTo(this.$body);
      if (this.$el.parents('header').hasClass('is-frosted')) {
         this.$nav.addClass('is-frosted');
      }
      this.$menu = jQuery('<ul class="fcx-mobile-menu"/>').appendTo(this.$nav);
      this.$el
         .children('ul')
         .children()
         .each(function () {
      ...
      this.$overlay = jQuery('<div class="fcx-mobile-menu-overlay"/>').appendTo(this.$body);
      ...
      // Events
      jQuery('.fcx-mobile-menu-toggler').on('click', function () {
...
class fcx_Infinite_List {
   constructor($obj = null, cfg = {}) {
      ...
      // Hover Event
      this.$el
         .on('mouseenter', '.fcx-infinite-list-item', function () {
            ...
            if ('video' === $preview.attr('data-type')) {
               jQuery(this).children('.fcx-infinite-list-preview').children('video')[0].play();
               $preview.children('video')[0].play();
            }
         })
         .on('mouseleave', '.fcx-infinite-list-item', function () {
            ...
            if ('video' === $preview.attr('data-type')) {
               jQuery(this).children('.fcx-infinite-list-preview').children('video')[0].pause();
   ...
   init() {
      ...
      // DOM
      this.$list = this.$el.children('.fcx-infinite-list');
      this.$bg_wrap = jQuery('<div class="fcx-infinite-list-bgs"/>').appendTo(this.$el);
      this.$scroll = jQuery('<div class="fcx-infinite-scroll"/>').appendTo(this.$el);
      this.$cloned_list = jQuery('<div class="fcx-infinite-scroll-inner"/>').appendTo(this.$scroll);
   ...
   clone_items() {
      ...
      this.$list.children().each(function (i) {
         ...
         if (_self.first_circle) {
            let $preview = $cloned.children('.fcx-infinite-list-preview');
            ...
            if ('video' === type) {
               ...
               _self.$bg_wrap.append(`
                    <div class="fcxl-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
            } else {
               $preview.css('background-image', 'url(' + this_src + ')');
               _self.$bg_wrap.append(
                  `<div class="fcxl-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`
               );
            }
         }
      });
...
class fcx_Marquee {
   constructor($obj = null) {
      ...
      this.width = this.$inner.find('.fcx-marquee-list').width();
   ...
   init() {
      this.$inner.wrapInner('<div class="fcx-marquee-inner-wrap"/>');
      this.$inner_cont = this.$inner.children('.fcx-marquee-inner-wrap');
...
class fcx_Masked {
   constructor($obj = null, cfg = {}) {
      ...
      this.cfg = Object.assign(
         {
            media_selector: '.fcx-masked-media',
            content_selector: '.fcx-masked-content',
            dbr: 24,
            ibr: 32,
         },
         cfg
...
class fcx_LWP {
   constructor($obj = null) {
      ...
      this.$media = jQuery('<div class="fcx-lwp-media" data-appear="fade-right"/>').prependTo(
         this.$el
      );
      this.count = this.$list.children('.fcx-lwp-item').length;
      this.active = 0;
      this.prev = 1;

      this.$list.children('.fcx-lwp-item').each(function (i) {
