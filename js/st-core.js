...
class ST_Core {
  constructor(o = {}) {
    ...
    this.cfg.header = {
      ...
      hide_on_scroll: this.$dom.header.hasClass("hide-on-scroll")
        ? true
        : false,
    };
    ...
    this.$dom.body.css({
      ...
      "--st-header-percent-height":
        parseFloat((this.sizes.header / this.sizes.win) * 100).toFixed(2) + "%",
      "--st-footer-height": this.sizes.footer + "px",
      "--st-footer-percent-height":
        parseFloat((this.sizes.footer / this.sizes.win) * 100).toFixed(2) + "%",
    ...
    if ("IntersectionObserver" in window) {
      ...
      // Threshold Observer
      this.threshold_observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (!entry.isIntersecting) {
              if (entry.target.hasOwnProperty("notInView")) {
  ...
  init() {
    ...
    if (this.dragProtection) {
      this.$dom.document.on("mousedown", "a", function (e) {
        if (
          jQuery(this).attr("href").indexOf(".png") ||
          jQuery(this).attr("href").indexOf(".gif") ||
          jQuery(this).attr("href").indexOf(".jpg")
        ) {
          e.preventDefault();
        }
      });
      ...
      jQuery("." + PREFIX + "-logo.is-retina").each(function () {
        ...
        $this.css({
          width: $this.children("img").attr("width")
            ? 0.5 * $this.children("img").attr("width")
            : _self.cfg.logo_size.w,
          height: $this.children("img").attr("height")
            ? 0.5 * $this.children("img").attr("height")
            : _self.cfg.logo_size.h,
        });
      });
    }
    ...
    // Smooth Scroll
    if (
      this.cfg.smoothScroll &&
      !this.isTouchDevice &&
      !this.$dom.body.hasClass("disable-smooth-scroll")
    ) {
    ...
    if (this.cfg.cursorFollow !== null && this.cfg.cursorFollow.length) {
      jQuery(document).on("mousemove", this.cfg.cursorFollow, function (e) {
        if (!_self.isTouchDevice) {
          ...
          jQuery(this).css(
            "transform",
            "translate(" + x * 0.33 + "px, " + y * 0.33 + "px) scale(1.1)"
          );
  ...
  lazyLoader(item) {
    ...
    if (_self.observer !== null) {
      $this
        .wrap('<div class="st-lazy-wrapper"/>')
        .removeClass(PREFIX + "-lazy");
      ...
      if (
        ...
        $this.attr(
          "src",
          `data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 ${$this.attr("width")} ${$this.attr("height")}' width='${$this.attr("width")}' height='${$this.attr("height")}'></svg>`
        );
      }
...
class ST_Smooth_Scroll {
  constructor($obj = null, core = null, options = {}) {
    ...
    this.options = {
      ...
      animateOnStop: options.hasOwnProperty("animateOnStop")
        ? options.animateOnStop
        : false,
    };

    // Define Callbacks
    this.onUpdate = options.hasOwnProperty("onUpdate")
      ? options.onUpdate
      : false;
  ...
  init() {
    ...
    this.$scroll.css(
      "transform",
      `translateY(${parseFloat(this.pos.c).toFixed(2)}px)`
    );
  ...
  updateScroll(deltaTime = 0) {
    ...
    if (this.options.animateOnStop && this.state === "stop") {
      ...
      if (
        parseFloat(this.pos.c).toFixed(1) === 0 &&
        parseFloat(this.pos.t).toFixed(1) === 0
      ) {
        this.changeState("stop");
        return false;
      } else if (
        parseFloat(this.pos.c).toFixed(1) != parseFloat(this.pos.t).toFixed(1)
      ) {
        this.state = "play";
        ...
        this.velocity =
          (this.pos.t - this.pos.c) *
          parseFloat(deltaTime * fpsChunk).toFixed(3) *
          this.options.speed;
...
class ST_SAP {
  constructor(core = null) {
    ...
    window.addEventListener("mousemove", (e) => {
      //-1 => 1
      _self.mouse_position.x =
        (2 * e.clientX - window.innerWidth) / window.innerWidth;
      _self.mouse_position.y =
  ...
  addParallaxMedia(elem = null, options = {}) {
    ...
    const item = {
      ...
      top: 0,
      height: 0,
      speed: options.hasOwnProperty("speed")
        ? parseInt(options.speed) * 0.01
        : 0.05,
      direction: options.hasOwnProperty("dir") ? options.dir : "vertical",
      mouse_cx: 0,
      calculate: function () {
        let offset_fix =
          _self.core.scroll !== null
            ? _self.core.scroll.$scroll.offset().top
            : 0;
        this.$el.css(
          "transform",
          "translateY(0px) scale(" + (1 + this.speed) + ")"
        );
  ...
  addHorizontalScrollItem(elem = null, options = {}) {
    if (elem === null) {
      console.warn(
        "Cannot add Horizontal Scroll Item. Element is not specified."
      );
      return false;
    }
    const _self = this;
    const item = {
      ...
      update: function (cs) {
        ...
        this.$el.css(
          "transform",
          `translate(-${this.scroll_path * t}px, ${sticky_path}px)`
        );
      },
    };
  ...
  addStickyItem(elem = null, options = {}) {
    ...
    const item = {
      ...
      position: options.hasOwnProperty("position")
        ? options.position
        : "center",
      calculate: function () {
        let offset_fix =
        ...
        if (_self.core.scroll !== null) {
          ...
          switch (this.position) {
            ...
            default:
              ...
              let top_offset = 0;
              if (
                this.position.indexOf("vw") > 0 ||
                this.position.indexOf("%") > 0
              ) {
                top_offset =
                  parseFloat(this.position) * 0.01 * _self.screen_height;
          ...
          switch (this.position) {
            ...
            default:
              if (
                ...
                this.$el.css(
                  "top",
                  `${parseFloat(this.position) * 0.01 * _self.screen_height}px`
                );
              } else if (parseFloat(this.position) <= 1) {
                // .%
                this.$el.css(
                  "top",
                  `${parseFloat(this.position) * _self.screen_height}px`
                );
              } else {
                // px
  ...
  addExpandItem(elem = null, options = {}) {
    ...
    const item = {
      ...
      calculate: function () {
        ...
        this.height = this.$el.height();
        (this.i_scale =
          this.$el.css("--st-eos-init-scale") !== undefined
            ? parseFloat(this.$el.css("--st-eos-init-scale"))
            : 0.75),
          (this.scale_path = 1 - this.i_scale),
          (this.i_clip =
            50 - (this.$el.css("--st-eos-init-clip") !== undefined)
              ? parseFloat(this.$el.css("--st-eos-init-clip")) * 50
              : 12.5), // 12.5 = 25% vissibility or 0.25 in styles; 50 - fully collapsed, 0 - fully visible
          (this.br =
            this.$el.css("--st-eos-br") !== undefined
              ? parseInt(this.$el.css("--st-eos-br"), 10)
              : 0),
          (this.start =
            this.$el.offset().top - offset_fix - _self.screen_height);
      ...
      update: function (cs) {
        ...
        this.$wrap.css(
          "clip-path",
          `inset(0% ${this.i_clip - this.i_clip * t}% round ${this.br}px)`
        );
        this.$el.css(
          "transform",
          "scale(" + (this.i_scale + this.scale_path * t) + ")"
        );
      },
    };
  ...
  addSpeedItem(elem = null, options = {}) {
    ...
    const item = {
      ...
      l_speed: options.hasOwnProperty("l_speed")
        ? parseFloat(options.l_speed) - 1
        : null,
      t_speed: options.hasOwnProperty("t_speed")
        ? parseFloat(options.t_speed) - 1
        : null,
      tp_speed: options.hasOwnProperty("tp_speed")
        ? parseFloat(options.tp_speed) - 1
        : null,
      m_speed: options.hasOwnProperty("m_speed")
        ? parseFloat(options.m_speed) - 1
        : null,
      calculate: function () {
  ...
  addTextFillMaskItem(elem = null, options = {}) {
    ...
    const item = {
      ...
      calculate: function () {
        ...
        this.height = this.$el.height();
        this.start =
          this.$el.offset().top - offset_fix - 0.9 * _self.screen_height;
        this.end =
          this.$el.offset().top -
          offset_fix -
          0.5 * _self.screen_height +
...
class ST_iCursor {
  constructor(core = null, options = {}) {
    ...
    const _self = this;
    this.core = core;
    this.$el = jQuery('<div class="' + PREFIX + '-cursor"/>').appendTo(
      jQuery("body")
    );
    ...
    if (window.localStorage.getItem(PREFIX + "_prev_cursor") !== null) {
      let prevCursor = JSON.parse(
        window.localStorage.getItem(PREFIX + "_prev_cursor")
      );
  ...
  update(delta = 0) {
    if (this.isActive) {
      this.position.lerpX +=
        (this.position.x - this.position.lerpX) * 10 * delta;
      this.position.lerpY +=
        (this.position.y - this.position.lerpY) * 10 * delta;
      this.$el.css(
        "transform",
        "translate(" +
          this.position.lerpX +
