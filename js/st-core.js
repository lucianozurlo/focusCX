...
class ST_SAP {
   ...
   addParallaxMedia(elem = null, options = {}) {
      ...
      const item = {
         ...
         calculate: function () {
            let offset_fix =
               _self.core.scroll !== null ? _self.core.scroll.$scroll.offset().top : 0;
         ...
         update: function (cs) {
            ...
            this.$el.css(
               'transform',
               'translateY(' + this.scrollPath * t + 'px) scale(' + (1 + this.speed) + ')'
            );
         },
      };
   ...
   addSpeedItem(elem = null, options = {}) {
      ...
      const item = {
         ...
         calculate: function () {
            ...
            this.height = this.$el.height();
            this.top =
               this.$el.offset().top - offset_fix - 0.5 * _self.screen_height + 0.5 * this.height;
            this.update(_self.core.getCurrentScroll());
         },
         update: function (cs) {
...
class ST_iCursor {
   ...
   update(delta = 0) {
      if (this.isActive) {
         ...
         this.$el.css(
            'transform',
            'translate(' + this.position.lerpX + 'px, ' + this.position.lerpY + 'px)'
         );
      }
   }
}
