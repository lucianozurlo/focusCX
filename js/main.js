...
const fcx_config = {
   ...
   // Cursor Following
   cursorFollow: '.fcx-socials-list a, .fcx-gallery-nav',
...
jQuery(function ($) {
   ...
   const core = new ST_Core(fcx_config);
   ...
   function MainMenuInit() {
      template.menu = new fcx_Menu(core);
   }

   // --- PHOTOSWITE LIGHTBOX --- //
   function initPSWP() {
      if (jQuery('.fcx-lightbox-link').length) {
         if (typeof PhotoSwipe === 'function') {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close fcx-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button fcx-pswp-nav fcx-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button fcx-pswp-nav fcx-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
            ...
            // Build PSWP Gallery
            jQuery('.fcx-lightbox-link').each(function () {
               ...
               if ($this.attr('data-type') && $this.attr('data-type') !== 'image') {
                  ...
                  if ('video' === $this.attr('data-type')) {
                     this_item.html =
                        '\
                            <div class="fcx-pswp-media--video" data-src="' +
                        $this.attr('href') +
                        '">\
                                <video src="' +
                        $this.attr('href') +
                     ...
                     this_item.html =
                        '\
                            <div class="fcx-pswp-media--iframe" data-player-src="' +
                        this_link +
                        '" data-src="' +
                        $this.attr('href') +
                        '" data-type="' +
            ...
            core.$dom.body.on('click', '.fcx-pswp-image-wrap', function (e) {
               e.preventDefault();
               e.stopPropagation();
            });
            core.$dom.body.on(
            ...
            jQuery(document).on('click', '.fcx-lightbox-link', function (e) {
               e.preventDefault();
               if (typeof PhotoSwipe === 'function') {
                  let $this = jQuery(this),
                           ...
                           $this_video
                              ...
                              .attr(
                                 ...
                                 $this_video.attr('data-player-src') +
                                    ...
                                    ($this_video.attr('data-type') === 'vimeo' ? 'muted' : 'mute') +
                                    '=1'
                              );
                        }
                     }
                  ...
                  template.pswp.lightbox.listen('resize', function () {
                     if (
                        jQuery(template.pswp.lightbox.container).find('.fcx-pswp-media--video')
                           .length
                     ) {
                        jQuery(template.pswp.lightbox.container)
                           .find('.fcx-pswp-media--video')
                           .width(template.pswp.resizeVideo().w)
                           .height(template.pswp.resizeVideo().h);
                     }
                     if (
                        jQuery(template.pswp.lightbox.container).find('.fcx-pswp-media--iframe')
                           .length
                     ) {
                        jQuery(template.pswp.lightbox.container)
                           .find('.fcx-pswp-media--iframe')
                           .width(template.pswp.resizeVideo().w)
                           .height(template.pswp.resizeVideo().h);
                     }
                  });
                           ...
                           .each(function () {
                              ...
                              if (template.inView(this)) {
                                 ...
                                 $this_video
                                    ...
                                    .attr(
                                       'src',
                                       jQuery(this).attr('data-src') + '?controls=1&amp;loop=0'
                                    );
                              }
                           });
                     }
            ...
            console.error('fcx Error: PSWP script not found.');
         }
      }
   }
   ...
   function HeroSections() {
      // Masked Blocks
      if (jQuery('.fcx-masked-block').length) {
         jQuery('.fcx-masked-block').each(function () {
            new fcx_Masked(this);
         });
      }
   }
   ...
   function BlocksHighlight() {
      if (jQuery('.fcx-backlight').length) {
         template.cursor_pos = {
            x: 0,
            y: 0,
         };
         jQuery(document).on('mousemove', function (e) {
            ...
            jQuery('.fcx-backlight').css({
               '--mouse-x': `${template.cursor_pos.x}px`,
               '--mouse-y': `${template.cursor_pos.y}px`,
            });
         });

         if (jQuery('.fcx-block').length) {
            jQuery('.fcx-block').each(function () {
   ...
   // --- MARQUEE ELEMENT --- //
   function Marquee() {
      if (jQuery('.fcx-marquee').length) {
         jQuery('.fcx-marquee').each(function () {
            new fcx_Marquee(this);
         });
      }
   }
      ...
      if (jQuery('.swiper').length) {
         ...
         const init_swiper = function () {
            ...
            jQuery('.swiper').each(function () {
               ...
               if (core.$dom.body.hasClass('fcx-fullscreen-page')) {
                  options.mousewheel = true;
               }
               // CAROUSEL
               if ($this.hasClass('fcx-carousel')) {
               ...
               if ($this.hasClass('fcx-slider')) {
                  ...
                  options.autoplay =
                     $this.attr('data-autoplay') !== undefined
                        ...
                        : false;
                  options.effect =
                     $this.attr('data-effect') !== undefined ? $this.attr('data-effect') : 'slide';
                  ...
                  // Slider Content
                  if ($this.find('.fcx-slide-content').length) {
                     ...
                     $this
                        ...
                        .each(function (i) {
                           ...
                           if ($slide.children('.fcx-slide-content').length) {
                              const $slide_content = $slide.children('.fcx-slide-content');
                              ...
                              $media.wrap('<div class="fcx-slider-media"/>');
               ...
               // Pagination
               if ($this.children('.fcx-dots').length) {
                  options.pagination = {
                     el: '.fcx-dots',
                     dynamicBullets: true,
               ...
               // Slider Navigation
               if ($this.parent().children('.fcx-slider-nav').length) {
                  let $this_nav = $this.parent().children('.fcx-slider-nav');
                  options.navigation = {
                     prevEl: $this_nav.children('.fcx-slider-prev')[0],
                     nextEl: $this_nav.children('.fcx-slider-next')[0],
                  };
         ...
         if (typeof Swiper === 'function') {
            ...
            console.error('fcx Error: Swiper Library not found.');
         }
      }
   }
   ...
   function CircleLinks() {
      if (jQuery('.fcx-circle-path-link:not(.is-init)').length) {
         jQuery('.fcx-circle-path-link:not(.is-init)').each(function () {
            let $this = jQuery(this),
               size =
   ...
   function LinksEvents() {
      jQuery('.fcx-back-to-listing').on('click', function () {
         window.localStorage.setItem('fcx_return_from', this.getAttribute('data-id'));
      });
      if (
         jQuery('.fcx-albums-listing').length &&
         window.localStorage.getItem('fcx_return_from') != false
      ) {
         jQuery(window).trigger('fcx_set_album', window.localStorage.getItem('fcx_return_from'));
         window.localStorage.setItem('fcx_return_from', 0);
      } else {
         window.localStorage.setItem('fcx_return_from', 0);
      }
   }
   ...
   function InfiniteList() {
      if (jQuery('.fcx-infinite-list-wrap:not(.is-init)').length) {
         jQuery('.fcx-infinite-list-wrap:not(.is-init)').each(function () {
            this.classList.add('is-init');
            core.updateRequired.push(new fcx_Infinite_List(this));
         });
      }
   }
   ...
   // --- COUNTERS --- //
   function Counters() {
      if (jQuery('.fcx-counter:not(.is-init)').length) {
         jQuery('.fcx-counter:not(.is-init)').each(function () {
            let $this = jQuery(this);
            this.classList.add('is-init');
            this.counter_count = function () {
               let $counter = $this.children('.fcx-counter-number');
   ...
   function ListWithPreview() {
      if (jQuery('.fcx-list-with-preview').length) {
         jQuery('.fcx-list-with-preview').each(function () {
            new fcx_LWP(this);
         });
      }
   }
   ...
   function BeforeAfter() {
      if (jQuery('.fcx-before-after:not(.is-init)').length) {
         jQuery('.fcx-before-after:not(.is-init)').each(function () {
            jQuery(this).addClass('is-init');
            core.updateRequired.push(new fcx_Before_After(jQuery(this)));
         });
      }
   }
   ...
   // --- TOGGLES --- //
   function Toggles() {
      if (jQuery('.fcx-toggles-item:not(.is-init)').length) {
         jQuery('.fcx-toggles-item:not(.is-init)').each(function () {
            let $this = jQuery(this);
            $this.addClass('is-init');
            $this
               .children('.fcx-toggles-item--title')
               .append('<i class="fcx-icon fcx-icon-arrow-down"/>');
            $this.children('.fcx-toggles-item--content').slideUp(1);
            $this.on('click', function () {
               jQuery(this).toggleClass('is-active');
               $this.children('.fcx-toggles-item--content').slideToggle(300);
            });
            $this.on('click', '.fcx-toggles-item--content', function (e) {
               e.stopPropagation();
               e.preventDefault();
            });
         });
   ...
   function ScrollTrigger() {
      ...
      if (jQuery('.fcx-inline-ribbon:not(.is-init)').length) {
         jQuery('.fcx-inline-ribbon:not(.is-init)').each(function () {
            if (core.sap !== null) {
               let $this = jQuery(this);
               $this.addClass('is-init');
      ...
      // Sticky Elements
      if (jQuery('.fcx-sticky-block:not(.is-init)').length) {
         jQuery('.fcx-sticky-block:not(.is-init)').each(function () {
            if (core.sap !== null) {
               let $this = jQuery(this);
               $this.addClass('is-init');
         ...
         jQuery('[data-speed]:not(.is-init)').each(function () {
            if (core.sap !== null) {
               ...
               core.sap.addSpeedItem($this, {
                  speed: spd,
                  l_speed:
                     $this.attr('data-l-speed') !== undefined ? $this.attr('data-l-speed') : spd,
                  t_speed:
                     $this.attr('data-t-speed') !== undefined ? $this.attr('data-t-speed') : spd,
                  tp_speed:
                     $this.attr('data-tp-speed') !== undefined ? $this.attr('data-tp-speed') : spd,
                  m_speed:
                     $this.attr('data-m-speed') !== undefined ? $this.attr('data-m-speed') : spd,
               });
            }
         });
      }
      ...
      // Expand on Scroll
      if (jQuery('.fcx-expand-on-scroll:not(.is-init)').length) {
         jQuery('.fcx-expand-on-scroll:not(.is-init)').each(function () {
            if (core.sap !== null) {
               let $this = jQuery(this);
               $this.addClass('is-init');
      ...
      // Text Mask Fill
      if (jQuery('.fcx-text-fill-mask:not(.is-init)').length) {
         jQuery('.fcx-text-fill-mask:not(.is-init)').each(function () {
            if (core.sap !== null) {
               let $this = jQuery(this);
               $this.addClass('is-init');
      ...
      // Parallax Media
      if (jQuery('.fcx-parallax-media:not(.is-init)').length) {
         jQuery('.fcx-parallax-media:not(.is-init)').each(function () {
            let $this = jQuery(this);
            if (
   ...
   function ContactForm() {
      if (jQuery('.fcx-contact-form').length) {
         // The code related to the contact form is in the file: js/contact_form.js
         if (typeof fcx_Contact_Form === 'function') {
            fcx_Contact_Form();
         } else {
            console.error('fcx Error: Contact form script not found.');
         }
      }
   }
   ...
   function InitAnimation() {
      ...
      template.checkURL = function (el) {
         ...
         if (this_href.indexOf('javascript') === 0) {
            ...
            jQuery(fcx_config.linksException).each(function () {
               if ($el.is(this)) {
                  result = false;
               }
            });
   ...
   function Layout() {
      ...
      core.$dom.nav.find('.fcx-menu-offset').removeClass('fcx-menu-offset');
      ...
      if (core.$dom.body.hasClass('fcx-fullscreen-page')) {
         ...
         core.$dom.main.removeClass('fcx-min-content');
         ...
         if (core.$dom.main.height() < minHeight) {
            core.$dom.main.addClass('fcx-min-content');
   ...
   $(document).ready(function () {
      // General UI
      RightClickProtection('.fcx-rcp-wrap');
      MainMenuInit();
      CircleLinks();
